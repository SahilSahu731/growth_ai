...
const timezoneSuggestions = ["UTC", "Asia/Kolkata", "Asia/Singapore", "Europe/London", "Europe/Berlin", "America/New_York", "America/Los_Angeles", "Australia/Sydney"]
    ...
    <div className="min-h-svh bg-[#101010] text-[#f2f2f0] lg:grid lg:grid-cols-[264px_minmax(0,1fr)]">
        <aside className="border-b border-[#292929] bg-[#151515] p-4 lg:min-h-svh lg:border-b-0 lg:border-r lg:p-5">
          <div className="mb-7 hidden px-2 lg:block">
            <h1 className="text-xl font-semibold tracking-[-.02em] text-[#f5f5f3]">Settings</h1>
            <p className="mt-1 text-sm leading-6 text-[#8a8a86]">Your GrowthAI preferences</p>
          </div>
          <label className="relative block">
            ...
            <input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter" && visibleTabs[0]) selectTab(visibleTabs[0].id)
              }}
          ...
          <nav aria-label="Settings sections" className="mt-4 grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:block lg:space-y-1" role="tablist">
            ...
            {!visibleTabs.length ? <p className="col-span-full px-3 py-8 text-center text-xs leading-5 text-neutral-400">No settings match “{query}”.</p> : null}
          </nav>

          <div className="mt-7 hidden border-t border-[#303030] px-3 pt-6 lg:block">
            <p className="text-[11px] font-medium uppercase tracking-[.12em] text-[#777773]">Signed in as</p>
            <p className="mt-2 truncate text-sm font-normal text-[#a5a5a0]">{user.email}</p>
          </div>
        </aside>
...
function GeneralPanel({ user, preferences }: { user: SettingsUser; preferences: AccountOverview["preferences"] }) {
  ...
  return <SettingsPanel id="general" eyebrow="Account" title="General" description="Your profile identity and regional preferences.">
    <Section title="Profile" description="Your name, photo, and email come from your connected Google account.">
      ...
      <SettingRow label="Full name"><ReadOnlyValue>{user.name}</ReadOnlyValue></SettingRow>
      <SettingRow label="Email address"><ReadOnlyValue>{user.email}</ReadOnlyValue></SettingRow>
      <SettingRow label="Sign-in method"><span className="inline-flex items-center gap-2 text-sm font-normal text-[#c8c8c4]"><span className="flex size-7 items-center justify-center rounded-full bg-[#252525] text-xs font-semibold text-[#e8e8e5]">G</span>Google OAuth</span></SettingRow>
    </Section>
...
function CoachPanel({ preferences }: { preferences: AccountOverview["preferences"] }) {
  const [state, action, pending] = useActionState(updateSettingsAction, {} as SettingsActionState)
  const tones = [
    { id: "supportive", title: "Supportive", text: "Gentle encouragement with extra context.", icon: HeartHandshake },
    { id: "balanced", title: "Balanced", text: "Warm, clear, and practical by default.", icon: Sparkles },
    { id: "blunt", title: "Direct", text: "Concise guidance that gets to the point.", icon: Zap },
  ] as const
  return <SettingsPanel id="coach" eyebrow="Conversation" title="AI coach" description="Choose how GrowthAI should communicate while keeping its safety boundaries unchanged.">
    <Section title="Conversation style" description="This changes tone—not the quality, privacy, or safety rules of your guidance.">
      <form action={action} className="p-5 sm:p-6">
        ...
        <fieldset className="grid gap-3 md:grid-cols-3">
          ...
          {tones.map((tone) => {
            ...
            return <label key={tone.id} className="relative cursor-pointer">
              <input type="radio" name="coachTone" value={tone.id} defaultChecked={preferences.coachTone === tone.id} className="peer sr-only" />
              <span className="flex h-full min-h-40 flex-col rounded-xl border border-[#303030] bg-[#151515] p-5 transition hover:border-[#444] peer-checked:border-primary/60 peer-checked:bg-[#1b2426] peer-checked:text-white peer-focus-visible:ring-2 peer-focus-visible:ring-primary/40 peer-checked:[&_.tone-check]:opacity-100">
                ...
                <span className="mt-6 text-base font-medium">{tone.title}</span>
                <span className="mt-2 text-sm font-normal leading-6 text-[#92928e]">{tone.text}</span>
    ...
    <Section title="What stays consistent" description="These principles apply in every conversation style.">
      <InfoRow icon={ShieldCheck} title="Grounded guidance" text="Suggestions are based on what you share, with uncertainty kept visible." />
...
function BillingPanel({ planTier }: { planTier: SettingsUser["planTier"] }) {
  ...
  return <SettingsPanel id="billing" eyebrow="Subscription" title="Plan & billing" description="Review your entitlement and manage subscription renewal securely.">
    <div className="border-y border-[#303030]">
      <div className="flex flex-col gap-5 bg-[#151515] px-6 py-7 text-white sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-xs font-medium uppercase tracking-[.14em] text-primary">Current plan</p><p className="mt-3 text-3xl font-semibold">{resolvedPlan.name}</p><p className="mt-2 max-w-md text-sm font-normal leading-6 text-[#94948f]">{resolvedPlan.description}</p></div>
        <div className="shrink-0 text-left sm:text-right"><p className="text-2xl font-semibold">{resolvedPlan.monthlyPrice ? `₹${resolvedPlan.monthlyPrice}` : "₹0"}</p><p className="mt-1 text-xs text-[#858581]">{resolvedPlan.monthlyPrice ? "per month" : "no recurring charge"}</p></div>
      </div>
      <div className="px-6 py-7"><ul className="grid gap-4 sm:grid-cols-2">{resolvedPlan.features.map((feature) => <li key={feature} className="flex gap-2.5 text-sm font-normal leading-6 text-[#b8b8b3]"><Check className="mt-1 size-4 shrink-0 text-primary" />{feature}</li>)}</ul><div className="mt-7 flex flex-wrap gap-3 border-t border-[#292929] pt-6">{planTier === "free" ? <UpgradeTrigger feature="GrowthAI Pro" className="inline-flex h-10 items-center rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground">Compare paid plans</UpgradeTrigger> : null}<Button asChild variant="outline" className="h-10 rounded-lg px-4 text-sm font-medium"><Link href="/billing">Manage billing <ChevronRight /></Link></Button></div></div>
    </div>
    <Section title="Payment security" description="GrowthAI never stores card numbers, CVV, or banking credentials.">
      <InfoRow icon={LockKeyhole} title="Hosted checkout" text="Payment details are entered directly on Razorpay’s secure checkout." />
      <InfoRow icon={ShieldCheck} title="Verified access" text="Paid features activate only after a signed provider event matches your subscription." />
    </Section>
  </SettingsPanel>
}
...
function SettingsPanel({ id, eyebrow, title, description, children }: { id: TabId; eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return <section id={`settings-panel-${id}`} role="tabpanel" aria-labelledby={`settings-tab-${id}`} className="mx-auto w-full max-w-5xl space-y-12 px-6 py-10 sm:px-10 lg:px-14 lg:py-14"><header><p className="text-xs font-medium uppercase tracking-[.14em] text-primary">{eyebrow}</p><h2 className="mt-3 text-3xl font-semibold tracking-[-.025em] text-[#f5f5f3] sm:text-4xl">{title}</h2><p className="mt-3 text-base font-normal leading-7 text-[#999994]">{description}</p></header>{children}</section>
}

function Section({ title, description, children }: { title: string; description?: string; children: React.ReactNode }) {
  return <section className="border-t border-[#303030]"><div className="border-b border-[#292929] py-6"><h3 className="text-lg font-semibold text-[#ededeb]">{title}</h3>{description ? <p className="mt-2 text-sm font-normal leading-6 text-[#858581]">{description}</p> : null}</div><div className="divide-y divide-[#292929]">{children}</div></section>
}

function SettingRow({ label, children, align = "center" }: { label: string; children: React.ReactNode; align?: "center" | "start" }) {
  return <div className={cn("grid gap-4 py-5 sm:grid-cols-[minmax(170px,.55fr)_minmax(0,1fr)] sm:py-6", align === "center" ? "sm:items-center" : "sm:items-start")}><p className="text-sm font-normal text-[#c5c5c0]">{label}</p><div className="min-w-0 sm:justify-self-stretch">{children}</div></div>
}

function ReadOnlyValue({ children }: { children: React.ReactNode }) {
  return <div className="flex min-h-11 items-center rounded-lg border border-[#343434] bg-[#171717] px-3 text-sm font-normal text-[#dededb]">{children}</div>
}

function InfoRow({ icon: Icon, title, text }: { icon: ComponentType<{ className?: string }>; title: string; text: string }) {
  return <div className="flex gap-4 py-6"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-[#1d1d1d]"><Icon className="size-4 text-[#aaa9a5]" /></span><div><p className="text-sm font-medium text-[#dededb]">{title}</p><p className="mt-1.5 text-sm font-normal leading-6 text-[#858581]">{text}</p></div></div>
}

function FormFooter({ state, pending, inset = true }: { state: SettingsActionState; pending: boolean; inset?: boolean }) {
  return <div className={cn("mt-6 flex min-h-11 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between", inset && "pb-1")}><div aria-live="polite">{state.error ? <p className="text-sm font-normal text-red-400">{state.error}</p> : state.success ? <p className="flex items-center gap-2 text-sm font-normal text-emerald-400"><Check className="size-4" />{state.success}</p> : null}</div><Button disabled={pending} className="h-10 self-start rounded-lg px-5 text-sm font-semibold sm:self-auto">{pending ? "Saving…" : "Save changes"}</Button></div>
}
