...
function PricingGrid({ compact, authenticated, currentPlan }: { compact: boolean; authenticated: boolean; currentPlan: CurrentPlan }) {
  ...
  return <section id="pricing" className={cn("mx-auto w-full max-w-7xl px-4 sm:px-6", compact ? "py-12" : "py-24")}>
    {!compact ? <div className="mx-auto mb-12 max-w-2xl text-center"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-400">Simple pricing</p><h2 className="mt-4 text-3xl font-black tracking-[-.04em] text-neutral-950 sm:text-5xl">The core growth loop stays free.</h2><p className="mt-4 text-base leading-7 text-neutral-500">Pro adds several life intentions, deeper history, optional integrations, and exports—not artificial pressure.</p></div> : null}
