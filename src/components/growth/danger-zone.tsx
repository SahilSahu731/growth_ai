...
export function DangerZone({ email }: { email: string }) {
  ...
  return <div className="space-y-3"><p className="text-sm leading-6 text-neutral-500">Download your data first if you want a copy. Permanent deletion removes your chats, goals, tasks, billing profile, and account.</p><a href="/api/account/export" className="inline-block rounded-full border border-neutral-300 px-4 py-2 text-sm text-neutral-800">Download my data</a><div className="flex max-w-xl flex-col gap-2 sm:flex-row"><Input value={confirmation} onChange={event => setConfirmation(event.target.value)} placeholder={email} aria-label="Confirm account email" /><Button type="button" variant="destructive" disabled={pending || confirmation !== email} onClick={() => void remove()}>{pending ? "Deleting…" : "Delete permanently"}</Button></div>{error ? <p className="text-sm text-red-600">{error}</p> : null}</div>
}
