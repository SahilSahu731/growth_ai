...
export function AdminComplimentaryGrantForm({ userId }: { userId: string }) {
  ...
  return <form action={action} className="space-y-3"><input type="hidden" name="userId" value={userId} /><input type="hidden" name="planTier" value="pro" /><div className="grid gap-3 sm:grid-cols-2"><div className={`${inputClass} flex items-center text-neutral-300`}>Pro access</div><select className={inputClass} name="source" defaultValue="design_partner"><option value="design_partner">Design partner</option><option value="admin_comp">Admin complimentary</option><option value="support_remediation">Support remediation</option></select><label className="space-y-1"><span className="text-xs text-neutral-500">Starts (UTC)</span><input className={inputClass} name="startsAt" type="datetime-local" required /></label><label className="space-y-1"><span className="text-xs text-neutral-500">Expires (UTC)</span><input className={inputClass} name="expiresAt" type="datetime-local" required /></label></div><textarea className={`${inputClass} min-h-24 py-3`} name="reason" minLength={10} maxLength={300} placeholder="Required business reason" required /><Feedback state={state} /><SubmitButton>Grant access</SubmitButton></form>
}
