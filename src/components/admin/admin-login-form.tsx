...
function CredentialField({ name, label, type, autoComplete, icon: Icon, placeholder, inputMode }: {
  name: string; label: string; type: "email" | "password" | "text"; autoComplete: string; icon: typeof Mail; placeholder: string; inputMode?: "numeric"
}) {
  return (
    <label className="block space-y-2">
      ...
      <span className="flex h-12 items-center gap-3 rounded-xl border border-[#2d3538] bg-[#111719] px-4 transition focus-within:border-cyan-300/60 focus-within:bg-[#141c1f] focus-within:ring-4 focus-within:ring-cyan-300/5">
        ...
        <input name={name} type={type} autoComplete={autoComplete} inputMode={inputMode} pattern={inputMode === "numeric" ? "[0-9]{6}" : undefined} maxLength={inputMode === "numeric" ? 6 : undefined} placeholder={placeholder} required className="admin-login-input min-w-0 flex-1 bg-transparent text-sm text-[#f5f7f7] caret-cyan-300 outline-none placeholder:text-[#647277]" />
      </span>
    </label>
  )
}
...
export function AdminLoginForm() {
  ...
  return (
    <form action={action} className="mt-8 space-y-5">
      ...
      <CredentialField name="otp" label="Authenticator code" type="text" autoComplete="one-time-code" inputMode="numeric" icon={KeyRound} placeholder="6-digit code" />
