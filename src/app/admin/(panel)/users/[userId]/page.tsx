...
export default async function AdminUserPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return <div className="space-y-6"><Link href="/admin/users" className="inline-flex items-center gap-2 text-xs font-bold text-neutral-500 hover:text-white"><ArrowLeft className="size-4" />Back to users</Link><section className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><span className="flex size-14 items-center justify-center rounded-2xl bg-primary/10 text-lg font-black text-primary">{user.name.slice(0, 2).toUpperCase()}</span><div><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-black tracking-tight text-white sm:text-3xl">{user.name}</h1><span className={`rounded-full px-2 py-1 text-[9px] font-bold uppercase ${suspended ? "bg-red-500/10 text-red-300" : "bg-emerald-500/10 text-emerald-400"}`}>{suspended ? "Suspended" : "Active"}</span></div><p className="mt-1 text-xs text-neutral-500">{user.email} · {user.authProvider} · joined {date(user.createdAt)}</p></div></div><span className="self-start rounded-xl border border-primary/20 bg-primary/5 px-4 py-2 text-xs font-bold capitalize text-primary sm:self-auto">{user.planTier} plan</span></section>
