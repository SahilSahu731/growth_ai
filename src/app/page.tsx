  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#fafafa] text-neutral-950">
    <div className="mx-auto w-full max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
      ...
      <section className="growth-grid relative flex min-h-[calc(100vh-72px)] flex-col items-center justify-center overflow-hidden pb-12 pt-16 text-center sm:pt-20">
        ...
        <div className="relative z-10 flex items-center gap-2 rounded-full border border-neutral-200 bg-white/85 px-3 py-1.5 text-xs font-bold text-neutral-600 shadow-sm backdrop-blur"><BrandLogo className="size-6 rounded-full" /><span>Personal growth using only relevant saved context</span></div>
      ...
      <section className="grid grid-cols-1 gap-5 py-12 sm:grid-cols-3">
        {[{ value: "3", label: "active goals on the free plan" }, { value: "5 min", label: "for an honest daily reflection" }, { value: "1", label: "focused next step at a time" }].map(item => <div key={item.label} className="motion-card rounded-2xl border border-neutral-200 bg-white/85 p-6 text-center shadow-sm"><p className="font-editorial text-5xl italic text-neutral-950 sm:text-6xl">{item.value}</p><p className="mt-2 text-[11px] font-bold uppercase tracking-[.14em] text-neutral-400">{item.label}</p></div>)}
      </section>
